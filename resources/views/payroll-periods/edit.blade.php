...
<div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                {{ app()->getLocale() === 'ar' ? 'تعديل فترة الرواتب' : 'Edit Payroll Period' }}
            </h1>
            <p class="mt-1 text-sm text-slate-500 font-medium">
                {{ app()->getLocale() === 'ar' ? 'تحديث المدى الزمني وتاريخ صرف الرواتب.' : 'Update the date range and payment schedule for this period.' }}
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden">
        <form action="{{ route('payroll-periods.update', $payrollPeriod->id) }}" method="POST" class="p-8 sm:p-12 space-y-10">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name_ar" id="name_ar" value="{{ old('name_ar', $payrollPeriod->name_ar) }}" required class="erp-input w-full p-4 rounded-2xl">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $payrollPeriod->name_en) }}" required class="erp-input w-full p-4 rounded-2xl" dir="ltr">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-3">
                    <div class="space-y-2">
                        ...
                        <input type="date" name="starts_on" id="starts_on" value="{{ old('starts_on', $payrollPeriod->starts_on->format('Y-m-d')) }}" required class="erp-input w-full p-4 rounded-2xl">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="date" name="ends_on" id="ends_on" value="{{ old('ends_on', $payrollPeriod->ends_on->format('Y-m-d')) }}" required class="erp-input w-full p-4 rounded-2xl">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="date" name="pay_date" id="pay_date" value="{{ old('pay_date', $payrollPeriod->pay_date->format('Y-m-d')) }}" required class="erp-input w-full p-4 rounded-2xl">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
                    <div class="space-y-2">
                        <label for="status" class="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest">{{ app()->getLocale() === 'ar' ? 'الحالة الحالية' : 'Current Status' }}</label>
                        <select name="status" id="status" class="erp-input w-full p-4 rounded-2xl bg-white dark:bg-slate-900">
                            <option value="open" {{ old('status', $payrollPeriod->status->value) === 'open' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'مفتوح (جاهز للتشغيل)' : 'Open' }}</option>
                            <option value="closed" {{ old('status', $payrollPeriod->status->value) === 'closed' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'مغلق' : 'Closed' }}</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-end gap-4">
                ...
                <button type="submit" class="inline-flex items-center justify-center px-10 py-4 text-sm font-black text-white bg-brand-600 hover:bg-brand-700 rounded-2xl shadow-xl shadow-brand-500/20 transition-all active:scale-95">
                    {{ app()->getLocale() === 'ar' ? 'تحديث الفترة' : 'Update Period' }}
                </button>
            </div>
        </form>
    </div>
