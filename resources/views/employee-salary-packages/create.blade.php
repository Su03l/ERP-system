...
<div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                {{ app()->getLocale() === 'ar' ? 'إنشاء حزمة راتب جديدة' : 'Create Salary Package' }}
            </h1>
            <p class="mt-1 text-sm text-slate-500 font-medium">
                {{ app()->getLocale() === 'ar' ? 'تخصيص الراتب الأساسي والبدلات الثابتة لكل موظف.' : 'Assign basic salary and fixed allowances to specific employees.' }}
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden">
        <form action="{{ route('employee-salary-packages.store') }}" method="POST" class="p-8 sm:p-12 space-y-10">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="employee_id" id="employee_id" required class="erp-input w-full p-4 rounded-2xl bg-white dark:bg-slate-900 border-slate-200 focus:ring-brand-500">
                        <option value="">{{ app()->getLocale() === 'ar' ? 'اختر موظفاً من القائمة' : '-- Select an Employee --' }}</option>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" name="basic_salary" id="basic_salary" value="{{ old('basic_salary') }}" required class="erp-input w-full p-4 rounded-2xl" dir="ltr">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" name="housing_allowance" id="housing_allowance" value="{{ old('housing_allowance', '0.00') }}" class="erp-input w-full p-4 rounded-2xl" dir="ltr">
                            <span class="absolute inset-y-0 right-4 flex items-center text-xs font-bold text-slate-400 pointer-events-none">SAR</span>
                        </div>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" name="transportation_allowance" id="transportation_allowance" value="{{ old('transportation_allowance', '0.00') }}" class="erp-input w-full p-4 rounded-2xl" dir="ltr">
                            <span class="absolute inset-y-0 right-4 flex items-center text-xs font-bold text-slate-400 pointer-events-none">SAR</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <input type="date" name="effective_from" id="effective_from" value="{{ old('effective_from') }}" required class="erp-input w-full p-4 rounded-2xl">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="date" name="effective_to" id="effective_to" value="{{ old('effective_to') }}" class="erp-input w-full p-4 rounded-2xl">
                    </div>
                    <div class="space-y-2">
                        <label for="status" class="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest">{{ app()->getLocale() === 'ar' ? 'حالة الباقة' : 'Initial Status' }}</label>
                        <select name="status" id="status" class="erp-input w-full p-4 rounded-2xl bg-white dark:bg-slate-900">
                            <option value="active">{{ app()->getLocale() === 'ar' ? 'نشطة (مفعلة)' : 'Active' }}</option>
                            <option value="ended">{{ app()->getLocale() === 'ar' ? 'منتهية' : 'Ended' }}</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-end gap-4">
                ...
                <button type="submit" class="inline-flex items-center justify-center px-10 py-4 text-sm font-black text-white bg-brand-600 hover:bg-brand-700 rounded-2xl shadow-xl shadow-brand-500/20 transition-all active:scale-95">
                    {{ app()->getLocale() === 'ar' ? 'حفظ الباقة' : 'Save Package' }}
                </button>
            </div>
        </form>
    </div>
