        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white leading-tight">
                    {{ app()->getLocale() === 'ar' ? 'تعديل حساب المستخدم' : 'Edit User Account' }}
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {{ app()->getLocale() === 'ar' ? 'تحديث البيانات الأساسية وتعديل الأدوار والمسؤوليات لحساب: ' : 'Edit general details and adapt assigned job roles for: ' }} <strong>{{ $user->name }}</strong>
                </p>
            </div>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ route('users.update', $user->id) }}" class="space-y-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="erp-card p-6 space-y-6">
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-3">
                        ...
                        <p class="text-xs text-slate-400 mt-1">{{ app()->getLocale() === 'ar' ? 'تحديث الاسم والبريد الإلكتروني، أو إدخال كلمة مرور جديدة لتغييرها.' : 'Core login credentials utilized for ERP system authentication.' }}</p>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-form-input name="name" :label="app()->getLocale() === 'ar' ? 'الاسم بالكامل' : 'Full Name'" :value="$user->name" required />
                        <x-form-input name="email" type="email" :label="app()->getLocale() === 'ar' ? 'البريد الإلكتروني المهني' : 'Work Email Address'" :value="$user->email" required />
                        
                        <div class="md:col-span-2 border-t border-slate-100 dark:border-slate-800/80 pt-4 mt-2">
                            <span class="text-xs font-bold text-rose-500 block mb-2">{{ app()->getLocale() === 'ar' ? 'تغيير كلمة المرور (اختياري)' : 'Change Password (Optional)' }}</span>
                        </div>
                        
                        <x-form-input name="password" type="password" :label="app()->getLocale() === 'ar' ? 'كلمة المرور الجديدة' : 'New Password'" :help-text="app()->getLocale() === 'ar' ? 'اتركه فارغاً للاحتفاظ بكلمة المرور الحالية.' : 'Leave empty to preserve current password.'" />
                        <x-form-input name="password_confirmation" type="password" :label="app()->getLocale() === 'ar' ? 'تأكيد كلمة المرور الجديدة' : 'Confirm New Password'" />
                    </div>
                </div>
            </div>
        ...
        <div class="sticky bottom-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-t border-slate-200/80 dark:border-slate-800/80 -mx-4 md:-mx-8 px-6 py-4 flex items-center justify-end gap-3 z-10">
            ...
            <button type="submit" class="btn-primary shadow-md shadow-brand-500/10 hover:shadow-brand-500/20 active:scale-98 transition-transform font-bold text-sm">
                ...
                <span>{{ app()->getLocale() === 'ar' ? 'حفظ التحديثات' : 'Save Changes' }}</span>
            </button>
        </div>
    </form>
</x-app-layout>
