        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white leading-tight">
                    {{ app()->getLocale() === 'ar' ? 'إنشاء دور جديد وتحديد الصلاحيات' : 'Create Custom Job Role' }}
                </h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {{ app()->getLocale() === 'ar' ? 'حدد اسم الدور التعريفي، وقم بمنح الصلاحيات الدقيقة عبر الوحدات المختلفة.' : 'Provide details and map permission matrices to formulate custom employee roles.' }}
                </p>
            </div>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ route('roles.store') }}" class="space-y-6">
        ...
        <div class="erp-card p-6 space-y-6">
            <div class="border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <p class="text-xs text-slate-400 mt-1">{{ app()->getLocale() === 'ar' ? 'يجب إدخال اسم فريد للدور ليظهر عند إسناده للموظفين.' : 'Name custom profiles clearly to facilitate assignment on team members.' }}</p>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-form-input name="name" :label="app()->getLocale() === 'ar' ? 'اسم الدور' : 'Role Name'" placeholder="e.g. HR Manager" required />
                <x-form-input name="key" :label="app()->getLocale() === 'ar' ? 'رمز المعرف الفريد' : 'Key Identifier'" placeholder="e.g. hr_manager" required />
                <div class="md:col-span-2">
                    <x-form-input name="description" :label="app()->getLocale() === 'ar' ? 'الوصف الوظيفي للمسؤوليات' : 'Role Description & Scope'" placeholder="Describe the access permissions and business responsibilities associated with this role." />
                </div>
            </div>
        </div>
                    ...
                    <div class="erp-card p-5 space-y-4 border border-slate-100 dark:border-slate-800/80">
                        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                            ...
                            <label class="inline-flex items-center gap-1.5 cursor-pointer text-xs font-medium text-slate-400 select-none">
                                <input type="checkbox" onchange="toggleModuleCheckboxes('{{ $module }}', this.checked)" class="rounded border-slate-300 text-brand-600 focus:ring-brand-500 w-3.5 h-3.5 cursor-pointer">
                            ...
                            @foreach($perms as $perm)
                                <div class="flex items-start gap-2">
                                    <input 
                                        ...
                                        value="{{ $perm->key }}"
                                        data-module="{{ $module }}"
                                        {{ is_array(old('permissions')) && in_array($perm->key, old('permissions')) ? 'checked' : '' }}
        ...
        <div class="sticky bottom-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-t border-slate-200/80 dark:border-slate-800/80 -mx-4 md:-mx-8 px-6 py-4 flex items-center justify-end gap-3 z-10">
            ...
            <button type="submit" class="btn-primary shadow-md shadow-brand-500/10 hover:shadow-brand-500/20 active:scale-98 transition-transform font-bold text-sm">
                ...
                <span>{{ app()->getLocale() === 'ar' ? 'حفظ الدور الجديد' : 'Save New Role' }}</span>
            </button>
        </div>
    </form>
