...
@section('title', app()->getLocale() === 'ar' ? 'تسجيل حضور وانصراف يدوي' : 'Manual Attendance Entry')
...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">
                {{ app()->getLocale() === 'ar' ? 'تسجيل حضور وانصراف يدوي' : 'Manual Attendance Entry' }}
            </h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {{ app()->getLocale() === 'ar' ? 'قم بإدخال سجل حضور الموظف يدوياً.' : 'Manually enter employee attendance record.' }}
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
        <form action="{{ route('attendance-records.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="date" name="attendance_date" id="attendance_date" value="{{ old('attendance_date', date('Y-m-d')) }}" required
                        class="erp-input w-full">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="erp-input w-full">
                        <option value="present" {{ old('status') === 'present' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'حاضر' : 'Present' }}</option>
                        <option value="absent" {{ old('status') === 'absent' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'غائب' : 'Absent' }}</option>
                        <option value="late" {{ old('status') === 'late' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'متأخر' : 'Late' }}</option>
                        <option value="on_leave" {{ old('status') === 'on_leave' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'في إجازة' : 'On Leave' }}</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="clock_in_at" id="clock_in_at" value="{{ old('clock_in_at') }}"
                        class="erp-input w-full">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="clock_out_at" id="clock_out_at" value="{{ old('clock_out_at') }}"
                        class="erp-input w-full">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="source" id="source" class="erp-input w-full">
                        <option value="manual" {{ old('source', 'manual') === 'manual' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'إدخال يدوي' : 'Manual' }}</option>
                        <option value="web" {{ old('source') === 'web' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'البوابة' : 'Web' }}</option>
                        <option value="mobile" {{ old('source') === 'mobile' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'الجوال' : 'Mobile' }}</option>
                        <option value="device" {{ old('source') === 'device' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'جهاز بصمة' : 'Device' }}</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3" class="erp-input w-full">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-3">
                ...
                <button type="submit" class="btn-primary px-6 py-2">
                    {{ app()->getLocale() === 'ar' ? 'حفظ السجل' : 'Save Record' }}
                </button>
            </div>
        </form>
    </div>
