        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white leading-tight">
                    {{ app()->getLocale() === 'ar' ? 'إضافة قسم جديد' : 'Add New Department' }}
                </h1>
            </div>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('departments.store') }}" class="space-y-6 max-w-4xl mx-auto">
        @csrf
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-brand-600 focus:ring-brand-600" {{ old('is_active', true) ? 'checked' : '' }}>
        ...
        <div class="flex items-center justify-end gap-4">
            ...
            <button type="submit" class="btn-primary px-8 py-2.5 shadow-md hover:shadow-lg transition-shadow">
                {{ app()->getLocale() === 'ar' ? 'حفظ القسم' : 'Save Department' }}
            </button>
        </div>
    </form>
</x-app-layout>
