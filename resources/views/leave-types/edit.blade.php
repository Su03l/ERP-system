...
@section('title', app()->getLocale() === 'ar' ? 'تعديل نوع إجازة' : 'Edit Leave Type')
...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">
                {{ app()->getLocale() === 'ar' ? 'تعديل نوع الإجازة' : 'Edit Leave Type' }}
            </h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {{ app()->getLocale() === 'ar' ? 'تحديث بيانات وسياسة الإجازة.' : 'Update leave policy details.' }}
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
        <form action="{{ route('leave-types.update', $leaveType->id) }}" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="name_ar" id="name_ar" value="{{ old('name_ar', $leaveType->name_ar) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $leaveType->name_en) }}" required
                ...
                <div>
                    ...
                    <input type="number" step="0.5" min="0" name="default_days_per_year" id="default_days_per_year" value="{{ old('default_days_per_year', $leaveType->default_days_per_year) }}" required
                        class="erp-input w-full" dir="ltr">
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="allow_negative_balance" id="allow_negative_balance" value="1" {{ old('allow_negative_balance', $leaveType->allow_negative_balance) ? 'checked' : '' }}
                        class="w-5 h-5 text-brand-600 rounded border-slate-300 focus:ring-brand-600">
                ...
                <div>
                    ...
                    <select name="status" id="status" class="erp-input w-full">
                        <option value="active" {{ old('status', $leaveType->status?->value) === 'active' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'نشط' : 'Active' }}</option>
                        <option value="inactive" {{ old('status', $leaveType->status?->value) === 'inactive' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'غير نشط' : 'Inactive' }}</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" class="erp-input w-full">{{ old('description', $leaveType->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-3">
                ...
                <button type="submit" class="btn-primary px-6 py-2">
                    {{ app()->getLocale() === 'ar' ? 'تحديث نوع الإجازة' : 'Update Leave Type' }}
                </button>
            </div>
        </form>
    </div>
