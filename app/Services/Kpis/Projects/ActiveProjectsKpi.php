...
class ActiveProjectsKpi implements KpiResolver
{
    use ResolvesKpiResults;

    public function definition(): KpiDefinition
    {
        return $this->definitionFor('projects.active_projects', 'projects', 'المشاريع النشطة', 'Active projects', 'projects.view');
    }

    public function resolve(Company $company, KpiDateRange $dateRange): KpiResult
    {
        $value = Project::query()->forCompany($company)->where('status', ProjectStatus::Active->value)->whereBetween('created_at', [$dateRange->start, $dateRange->end])->count();
