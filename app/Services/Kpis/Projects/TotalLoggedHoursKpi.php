...
class TotalLoggedHoursKpi implements KpiResolver
{
    use ResolvesKpiResults;

    public function definition(): KpiDefinition
    {
        return $this->definitionFor('projects.total_logged_hours', 'projects', 'إجمالي الساعات المسجلة', 'Total logged hours', 'project_time_logs.view');
    }

    public function resolve(Company $company, KpiDateRange $dateRange): KpiResult
    {
        $minutes = (int) ProjectTimeLog::query()->forCompany($company)->whereDate('log_date', '>=', $dateRange->start->toDateString())->whereDate('log_date', '<=', $dateRange->end->toDateString())->sum('total_minutes');
