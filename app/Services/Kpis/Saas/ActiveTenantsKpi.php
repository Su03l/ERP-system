...
class ActiveTenantsKpi implements KpiResolver
{
    use ResolvesKpiResults;

    public function definition(): KpiDefinition
    {
        return $this->definitionFor('saas.active_tenants', 'saas', 'المستأجرون النشطون', 'Active tenants', 'subscriptions.view');
    }

    public function resolve(Company $company, KpiDateRange $dateRange): KpiResult
    {
        $value = CompanySubscription::query()->where('status', SubscriptionStatus::Active->value)->distinct('company_id')->count('company_id');
