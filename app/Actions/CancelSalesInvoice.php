...
class CancelSalesInvoice
{
    ...
    public function handle(SalesInvoice $invoice, ?User $actor = null, ?string $reason = null): SalesInvoice
    {
        $actor = $this->actor($actor);
        $this->ensureTenant($invoice, $actor);
        Gate::forUser($actor)->authorize('sales_invoices.cancel');

        return DB::transaction(function () use ($actor, $invoice, $reason): SalesInvoice {
            if ($invoice->posted_journal_entry_id !== null || in_array($invoice->status, [InvoiceStatus::Paid, InvoiceStatus::Cancelled, InvoiceStatus::Voided], true)) {
                throw ValidationException::withMessages([
                    'status' => __('accounting.validation.sales_invoices.cancelable_status'),
                ]);
            }
            ...
            $this->auditLogger->log('sales_invoice.cancelled', $invoice, $oldValues, $invoice->refresh()->attributesToArray(), ['reason' => $reason], $actor, $invoice->company_id);
    ...
    private function actor(?User $actor): User
    {
        ...
        if (! $actor instanceof User) {
            throw new AuthorizationException('An authenticated user is required to cancel sales invoices.');
        }
