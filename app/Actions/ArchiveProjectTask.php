...
class ArchiveProjectTask
{
    ...
    public function handle(ProjectTask $task, ?User $actor = null): ProjectTask
    {
        ...
        $this->authorize($actor, 'project_tasks.delete', $task->company_id);

        return DB::transaction(function () use ($task, $actor): ProjectTask {
            $oldValues = $task->attributesToArray();
            $project = $task->project;
            $task->delete();
            $this->auditLogger->log('project_task.archived', $task, $oldValues, $task->attributesToArray(), user: $actor, company: $task->company_id);
            $this->recalculateProjectProgress->handle($project, $actor);
    ...
    private function actor(?User $actor): User
    {
        ...
        if (! $actor instanceof User) {
            throw new AuthorizationException('An authenticated user is required to archive project tasks.');
        }
