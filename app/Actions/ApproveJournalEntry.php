...
class ApproveJournalEntry
{
    ...
    public function handle(JournalEntry $journalEntry, ?User $actor = null, ?string $comment = null): JournalEntry
    {
        ...
        Gate::forUser($actor)->authorize('journal_entries.approve');

        return DB::transaction(function () use ($actor, $comment, $journalEntry): JournalEntry {
            $this->ensureApprovable($journalEntry);
     ...
     */
    private function ensureApprovable(JournalEntry $journalEntry): void
    {
        if (! in_array($journalEntry->status, [JournalEntryStatus::Draft, JournalEntryStatus::PendingApproval], true)) {
            throw ValidationException::withMessages([
                'status' => __('accounting.validation.journal_entries.approvable_status'),
            ]);
        }
    }
    ...
    private function actor(?User $actor): User
    {
        ...
        if (! $actor instanceof User) {
            throw new AuthorizationException('An authenticated user is required to approve journal entries.');
        }
